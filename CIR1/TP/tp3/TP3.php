    ...
    <body>
		...
		<?php
			...
			function verificationEmail($mail){
				$flag = "<p  style='color:red';>Le mail n'est pas conforme.</p>";
				$end = "junia.com";
			...
			function verificationPassword($pass){
				$flag = "<p  style='color:red';>Le pass n'est pas conforme.</p>";
				if(strlen($pass) >= 8){
			...
			if(isset($pass) and isset($conf)){
				if($pass != $conf){
					$confErr = "<p style='color:red';>Les deux mdp ne sont pas égaux</p>";
				}
			}
			}
		?>
